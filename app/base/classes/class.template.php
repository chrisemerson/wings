 ...
 class Template {
  ...
  private function initialiseBlock ($strBlockName = "") {
   ...
   foreach ($arrLines as $intLineNo => $strLine) {
    ...
    if (preg_match(constant(__CLASS__ . "::RE_BLOCK_END"), $strLine, $arrMatches)) {
    ...
    if (!$blnCurrentlyInsideSubBlock && preg_match(constant(__CLASS__ . "::RE_INCLUDE"), $strLine, $arrMatches)) {
    ...
    if (preg_match(constant(__CLASS__ . "::RE_BLOCK_START"), $strLine, $arrMatches)) {
     if ($intBlockStack == 0) {
  ...
  private function parseIncludes ($strBlockName = "") {
   ...
   foreach ($arrLines as $intLineNo => $strLine) {
    ...
    if (preg_match(constant(__CLASS__ . "::RE_INCLUDE"), $strLine, $arrMatches)) {
    ...
    if (preg_match(constant(__CLASS__ . "::RE_INCLUDE_VARIABLE"), $strLine, $arrMatches)) {
    ...
    if ($blnIncludeFoundThisLoop) {
     $objIncludedTemplate = new Template($strIncludedTemplateName);
